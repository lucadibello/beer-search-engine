...
class Indexer:
  ...
  def __init__(self, index_name: str, index_destination_path: str, dataset_path: str):
  ...
  def _load_jsonl_dataset(self, dataset_path: str) -> pd.DataFrame:
      ...
      documents = [{'docno': f"d{i+1}", 'text': process_json_object(obj)} for i, obj in enumerate(json_objects)]
  ...
  def create_index(self, overwrite = False):
      # Paths join
      indexer = pt.DFIndexer(os.path.join(self.index_destination_path, self.index_name), overwrite=overwrite)
